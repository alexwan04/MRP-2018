...
% Using training set to predict the testing set
clear all;
...
Xtrain = Tensor( gender_index, month_index,day_of_week_index, NON_GIM_ERConsult_index, Waittime_to_admit_index,...
...
load experiment/smhtrain_mat

i=1;
while i <= 12278
    ind = smhtrain_mat(i,:);
    Xtrain.data(ind(:,1), ind(:,2), ind(:,3), ind(:,4), ind(:,5), ind(:,6), ind(:,7), ind(:,8), ind(:,9), ind(:,10)) = ind(:,11);
    i = i + 1;
end
Xtrain.data(Xtrain.data==0)=0.01;
...
factorization_model = {Xtrain, {Z1, Z2, Z3, Z4, Z5, Z6, Z7, Z8, Z9, Z10}}; 
